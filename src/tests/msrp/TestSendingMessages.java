 ...
 * 
 */
public class TestSendingMessages {
	InetAddress address;
	...
	public void test300KBMessageFileToMemory() {
		try {
			...
			Message threeHKbMessage = new FileMessage(sendingSession,
					"plain/text", tempFile);
	...
	public void test300KBMessageFileToFile() {
		try {
			...
			if (tempFileDir != null) {
				...
				receivingTempFile = File.createTempFile(Long.toString(System
						.currentTimeMillis()), null, null);

			FileDataContainer newFileDC = new FileDataContainer(
					receivingTempFile);
			...
			FileDataContainer fileDC = (FileDataContainer) receivingSessionListener
					.getReceiveMessage().getDataContainer();

			FileInputStream receivedFileStream = new FileInputStream(fileDC
					.getFile());
